...
public class JdeeMojo extends AbstractMojo {
  ...
  private Set getProjectArtifacts() throws InvalidVersionSpecificationException {
    ...
    for (Iterator dependencies
           = executedProject.getDependencies().iterator();
      ...
      if (scope == null) { scope = Artifact.SCOPE_COMPILE; }
  ...
  private Map createManagedVersionMap(ArtifactFactory artifactFactory,
    ...
    Map map;
    if (dependencyManagement != null && dependencyManagement.getDependencies()
        != null) {
      map = new HashMap();
      for (Iterator i = dependencyManagement.getDependencies().iterator();
        ...
        try {
          ...
          Artifact artifact = artifactFactory
            .createDependencyArtifact(d.getGroupId(), d.getArtifactId(),
  ...
  protected IdeDependency[] doDependencyResolution(MavenProject project)
   ...
   List deps = project.getDependencies();
    ...
    if (deps != null) {
      Map managedVersions
        = createManagedVersionMap(artifactFactory, project.getId(),
      ...
      while (pro != null) {
        ...
        for (Iterator iter = pro.getDependencies().iterator();
             iter.hasNext();) {
      ...
      try {
        ...
        artifactResolutionResult
          = artifactCollector.collect(getProjectArtifacts(),
                                      project.getArtifact(),
                                      managedVersions,
                                      localRepo,
      ...
      for (Iterator i = artifactResolutionResult
             .getArtifactResolutionNodes().iterator(); i.hasNext(); ) {
        ResolutionNode node = (ResolutionNode) i.next();
        int dependencyDepth = node.getDepth();
        Artifact art = node.getArtifact();

        try {
          artifactResolver.resolve(art, node.getRemoteRepositories(),
                                   localRepo);
        ...
        if (emittedReactorProjectId.add(art.getGroupId() + '-'
                                        + art.getArtifactId())) {

          boolean firstLevel = firstLevelDep.contains(art.getGroupId() + ":"
                                                      + art.getArtifactId());
    ...
    IdeDependency[] ideDeps = (IdeDependency[]) dependencies
      .toArray(new IdeDependency[dependencies.size()]);
    return ideDeps;
  }
   ...
   * @version 1.2.2
   */
  private void createJdeePrj(File sourceDir, File outputDir,
                             IdeDependency[] deps,
                             boolean test)
    ...
    StringBuffer javadoc
      = new StringBuffer("  '(jde-help-docsets (append '(\n");
    StringBuffer dependSourcePath
      = new StringBuffer("  '(jde-depend-sourcepath '(\n");
    ...
    for (int i = 0; i < deps.length; i++) {
      ...
      if (deps[i].isFirstLevel()) {
        getLog().debug("First level dependecies: " + deps[i].getGroupId()
                       + ":" + deps[i].getArtifactId());
        ...
        if (dependJavadocs) {
          ...
          if ((javadocPath == null || "".equals(javadocPath))
              && (generateMissingJavadoc
                  && sourcePath != null && !"".equals(sourcePath))) {
  ...
  private File dependecyCachePath(IdeDependency dependency) {
    ...
    if (path == null || "".equals(path)) {
      path = executedProject.getBasedir() + File.separator
        + ".maven-emacs-plugin-cache";
    }
    ...
    if (attachment != null && attachment.exists()) {
      ...
      if (f.listFiles() == null || f.listFiles().length == 0) {
        try {
          ...
          getLog().error("Cannot unzip file: " + attachment.getAbsolutePath()
   ...
   * @version 1.2.1
   */
  private String generateJavadoc(IdeDependency dependency, String sourcePath,
                                 String subDir) {
    String result;
    File f = dependecyCachePath(dependency);
    if (subDir != null && !"".equals(subDir)) {
    ...
    if (f.list() == null || f.list().length == 0) {
      List javadocParams
        = new LinkedList(Arrays.asList("-d", f.getAbsolutePath(), "-docletpath",
      ...
      try {
        ...
        StringBuffer parameters = new StringBuffer(javaHome
                                                   + "/../bin/javadoc ");
    ...
    for (Iterator iter = executedProject.getBuildPlugins().iterator();
      ...
      if (Plugin.class.isAssignableFrom(o.getClass())) {
        ...
        if ("org.apache.maven.plugins".equals(plugin.getGroupId())
          ...
          if (Xpp3Dom.class.isAssignableFrom(plugin.getConfiguration()
                                             .getClass())) {
  ...
  private List resolveDependenciesWithClassifier(IdeDependency[] deps,
    ...
    List remoteRepos = useRemoteRepos ? getRemoteArtifactRepositories()
  ...
  private void resolveSourceAndJavadocArtifacts(IdeDependency[] deps) {
    final List missingSources
      = resolveDependenciesWithClassifier(deps, "sources",
                                          isDownloadSources());
    //    missingSourceDependencies.addAll(missingSources);
    final List missingJavadocs
      = resolveDependenciesWithClassifier(deps, "javadoc",
                                          isDownloadJavadocs());
  ...
  public void execute() throws MojoExecutionException {
    ...
    try {
      ...
      getLog().debug("Count of dependecies: "
                     + Integer.toString(deps.length));
   ...
   */
  public final void setJdeeProjectFileVersion(final boolean
                                              newJdeeProjectFileVersion) {
